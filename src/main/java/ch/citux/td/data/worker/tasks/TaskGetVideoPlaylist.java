...
public class TaskGetVideoPlaylist extends TDTask<String, VideoPlaylist> {


    public TaskGetVideoPlaylist(TDCallback<VideoPlaylist> callback) {
        super(callback);
    }

    @Override
    protected VideoPlaylist doInBackground(String... params) {
        if (params.length == 1) {
            return TDServiceImpl.getInstance().getVideoPlaylist(params[0]);
        }
